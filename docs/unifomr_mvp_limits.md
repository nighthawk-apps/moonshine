...
2. **Clue PK directory** — always `found=true` + decoy PK + timing pad; clients **must** verify ownership. Moonshine rejects decoys (strict UnifOMR); Nighthawk can supplemental trial-decrypt when `strict_omr_only` is off.
