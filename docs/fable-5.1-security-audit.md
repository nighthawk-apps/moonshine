...
**Shipped revision:** `0441a57` (v3.0.14)  
...
| M3 | Non-ASCII memo / address print panic | **FIXED** | Display paths use safe UTF-8 handling (v3.0.14) |
| M4 | `--token` ignored; always DRK | **FIXED** | `main.rs` rejects non-DRK `--token` with clear error |
| Doc budgets | `unifomr_mvp_limits.md` stale MiB figures | **PENDING** | Align docs with LWD 160 MiB / ~120 MiB keys |
| Strict OMR parity | Moonshine TD-only vs Nighthawk PIR+TD | **PENDING** | Document intentional difference (SHOULD) |
